...
import type { Confidence, FafData, FafScore, SectionScore } from '@/core/types';
...
export class ScoreCalculator {
  calculate(data: FafData): FafScore {
    ...
    if (data.ai_score && data.ai_scoring_system === '2025-08-30') {
      const embeddedScore = parseInt(data.ai_score.toString().replace('%', ''), 10);
      return {
  ...
  private calculateSectionScores(_data: FafData): Readonly<Record<string, SectionScore>> {
    return {
      project_components: {
        percentage: 0,
        filled: 0,
  ...
  private generateSuggestions(_data: FafData, filled: number, total: number): readonly string[] {
  ...
  private getConfidenceLevel(score: number): Confidence {
